...
export async function POST(request: NextRequest) {
  ...
  try {
    ...
    const { patientId, doctorId, receptionistId, etageId, blocId, salleId, date, time, duration, reason, notes, status } = data;
    ...
    let resolvedReceptionistId = user.role === 'receptionniste' ? user.userId : null;

    if (user.role === 'admin' && receptionistId) {
      const receptionist = await prisma.receptionist.findUnique({
        where: { id: receptionistId },
      });
      if (!receptionist) {
        return NextResponse.json(
          { message: 'RǸceptionniste non trouvǸ' },
          { status: 404 }
        );
      }
      resolvedReceptionistId = receptionistId;
    }
    ...
    const appointment = await prisma.appointment.create({
      data: {
        patientId,
        doctorId,
        receptionistId: resolvedReceptionistId || null,
