...
import { Plus, Calendar as CalendarIcon, Clock, CheckCircle, XCircle, Building2, Layers, DoorOpen, Search, Download, Trash2, LayoutGrid, History, Pencil } from 'lucide-react';
...
export default function AppointmentsPage() {
  ...
  // Calculer les statistiques (selon l'onglet actif)
  const totalAppointments = displayedAppointments.length;
  const scheduledAppointments = displayedAppointments.filter(a => a.status === 'scheduled').length;
  const completedAppointments = displayedAppointments.filter(a => a.status === 'completed').length;
  const cancelledAppointments = displayedAppointments.filter(a => a.status === 'cancelled').length;
                    ...
                    <td className="px-4 py-3">
                      <div className="flex items-center space-x-2">
                        ...
                        <button
                          ...
                          className="p-2 bg-[#FBBF24] text-white rounded-lg hover:bg-[#F59E0B] transition-all"
                          title="Annuler"
                        >
...
function AppointmentModal({
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (isAdmin) appointmentData.receptionistId = formData.receptionistId || null;
          ...
          {isAdmin && (
            <div>
              ...
              {loadingReceptionists ? (
                <div className="w-full px-3 py-2 border border-gray-300 rounded-lg bg-gray-50 text-gray-500">
                  Chargement des receptionnistes...
                </div>
              ) : receptionists.length === 0 ? (
                <div className="w-full px-3 py-2 border border-yellow-300 rounded-lg bg-yellow-50 text-yellow-700">
                  Aucun receptionniste disponible
                </div>
              ) : (
                <select
                  value={formData.receptionistId}
                  onChange={(e) => setFormData({ ...formData, receptionistId: e.target.value })}
                  className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:ring-2 focus:ring-[#3B9AEE] focus:border-transparent"
                >
                  <option value="">Selectionner un receptionniste (optionnel)</option>
